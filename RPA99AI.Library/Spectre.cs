...
namespace RPA99AI.Library
{
    public class Spectre
    {
        ...
        private static double GetSa2(double t, double a, double t1, double t2, double q, double r, double eta)
        {
            if (t >= 0.0 && t < t1 ) return 1.25 * a * (1.0 + (t / t1 * ((2.5 * eta * (q / r)) - 1.0)));
            if (t >= t1  && t < t2 ) return 1.25 * a * 2.5 * eta * (q / r);
            if (t >= t2  && t < 3.0) return 1.25 * a * 2.5 * eta * (q / r) * Pow(t2 / t, 2.0 / 3.0);
            if (t >= 3.0           ) return 1.25 * a * 2.5 * eta * (q / r) * Pow(t2 / 3.0, 2.0 / 3.0) * Pow(3.0 / t, 5.0 / 3.0);
